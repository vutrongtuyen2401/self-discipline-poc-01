...
|:---:|:---|:---:|:---|:---|:---:|:---|
| **OPEN-01** | Công thức giải phong ấn "2/3 nhiệm vụ" | **OPEN** | Cơ chế “hoàn thành 2/3 nhiệm vụ” được dùng làm hướng mở khóa. Tuy nhiên, cách tính chính xác cho từng số lượng nhiệm vụ (đặc biệt 1, 2, 3, 4… nhiệm vụ), làm tròn lên hay làm tròn xuống, và các trường hợp đặc biệt chưa được đặc tả thành công thức cuối cùng. | Phase 18 đã xây dựng Core Task Domain và luồng hoàn thành nhiệm vụ theo chu kỳ 04:00, nhưng `PolicyEngine.kt` và App Lock vẫn giữ nguyên phân định theo Schedule và Daily Limit, chưa có logic giải phong ấn theo tỷ lệ 2/3. | **CHƯA XÁC ĐỊNH** *(Đã có task domain, chưa gắn tỷ lệ 2/3 vào lock)* | **TUYỆT ĐỐI KHÔNG** tự chọn công thức toán học (`ceil(2N/3)`, `floor(2N/3)`, hay `round`). Phải chờ Ký chủ xác nhận bảng quy đổi cụ thể cho từng N. |
...
| **OPEN-05** | Lược đồ Cơ sở Dữ liệu & Chiến lược Di chuyển (Migration) | **OPEN** | Chi tiết schema cơ sở dữ liệu quan hệ chính thức (Room DB: TaskEntity, AppEntity, TaskAppCrossRef, VoucherEntity...) và chiến lược migration/tương thích từ DataStore hiện tại sang SQLite/Room. | Phase 17 và 18 sử dụng Room Database (`AppDatabase`, `TaskDao`, `DailyTaskCompletionDao`) cho Core Task Domain và mốc 04:00. DataStore tiếp tục duy trì 100% cho App Lock runtime. | **PROPOSAL (TECHNICAL FOUNDATION)** *(OPEN-05 REMAINS OPEN)* | **BẮT BUỘC:** Room Database hiện tại là Nền tảng kỹ thuật phục vụ Checkpoint CP3/CP5/CP6, **KHÔNG ĐƯỢC ĐÓNG OPEN-05**. Schema chính thức và chiến lược di trú toàn diện cần Ký chủ phê duyệt trước khi áp dụng cho toàn bộ sản phẩm. |
