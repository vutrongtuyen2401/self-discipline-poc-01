...
|:---:|:---|:---:|:---|:---|:---:|:---|
| **OPEN-01** | Công thức giải phong ấn "2/3 nhiệm vụ" | **OPEN** | Cơ chế “hoàn thành 2/3 nhiệm vụ” được dùng làm hướng mở khóa. Tuy nhiên, cách tính chính xác cho từng số lượng nhiệm vụ (đặc biệt 1, 2, 3, 4… nhiệm vụ), làm tròn lên hay làm tròn xuống, và các trường hợp đặc biệt chưa được đặc tả thành công thức cuối cùng. | Phase 19 đã tích hợp liên kết Task ↔ App trong Bảo Khố và xây dựng ranh giới an toàn `TaskAppEnforcementAdapter` trả về `DISABLED_PENDING_OPEN_01` (`isTaskBasedUnlockApproved = false`). Tuyệt đối không tự ý phát minh công thức 2/3, không tính tỷ lệ % hay điểm thưởng. | **CHƯA XÁC ĐỊNH** *(Cô lập ranh giới adapter an toàn, không kích hoạt)* | **TUYỆT ĐỐI KHÔNG** tự chọn công thức toán học (`ceil(2N/3)`, `floor(2N/3)`, hay `round`). Phải chờ Ký chủ xác nhận bảng quy đổi cụ thể cho từng N. |
