...
| **OPEN-05** | Lược đồ Cơ sở Dữ liệu & Chiến lược Di chuyển (Migration) | **OPEN** | Chi tiết schema cơ sở dữ liệu quan hệ chính thức (Room DB: TaskEntity, AppEntity, TaskAppCrossRef, VoucherEntity...) và chiến lược migration/tương thích từ DataStore hiện tại sang SQLite/Room. | Phase 17 đã xây dựng đề xuất Room Database (`AppDatabase`, `AppEntity`, `TaskEntity`, `TaskAppCrossRef`, `DailyTaskCompletionEntity`, `CoreDataRepository`) làm Technical Foundation (chi tiết tại [`docs/ROOM_SCHEMA_PROPOSAL.md`](file:///c:/Code/self-discipline-poc-01/docs/ROOM_SCHEMA_PROPOSAL.md)). DataStore tiếp tục duy trì 100% cho App Lock runtime. | **PROPOSAL (TECHNICAL FOUNDATION)** *(OPEN-05 REMAINS OPEN)* | **BẮT BUỘC:** Phase 17 chỉ đưa ra đề xuất kiến trúc dữ liệu kỹ thuật, **KHÔNG ĐƯỢC ĐÓNG OPEN-05**. Schema chính thức và chiến lược di trú toàn diện cần Ký chủ phê duyệt trước khi áp dụng cho toàn bộ sản phẩm. |
