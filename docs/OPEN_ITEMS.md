...
| **OPEN-05** | Lược đồ Cơ sở Dữ liệu & Chiến lược Di chuyển (Migration) | **OPEN** | Chi tiết schema cơ sở dữ liệu quan hệ chính thức (Room DB: TaskEntity, AppEntity, TaskAppCrossRef, VoucherEntity...) và chiến lược migration/tương thích từ DataStore hiện tại sang SQLite/Room. | Code hiện tại lưu trữ danh sách target app và policy dạng JSON qua Jetpack DataStore Preferences (`TargetRepositoryImpl.kt`). | **CHƯA XÁC ĐỊNH** *(Tạm thời dùng DataStore)* | **TUYỆT ĐỐI KHÔNG** tự coi DataStore là Database chính thức của sản phẩm. Việc chuyển đổi sang Room DB phải có schema được phê duyệt. |
