...
|:---|:---:|:---|:---|:---|
| **1. Hệ thống Phong Ấn** | **PARTIAL** | `AppDetectorAccessibilityService.kt`, `BlockingShieldOverlay.kt`, `LockScreenActivity.kt`, `TaskAppEnforcementAdapter.kt`. | Đã tích hợp thành công ranh giới Mission Domain + Vault Domain vào App Lock Core qua `TaskAppEnforcementAdapter`. Đánh giá 2 tầng (Technical Lock ưu tiên -> Vault/Task evaluation), in-memory snapshot an toàn cho Main Thread. Thiếu hiển thị task list trên màn hình khóa (bị chặn bởi OPEN-01). | Hoàn thành tầng tích hợp trong Phase 20; chờ chốt OPEN-01 để hiển thị tiến độ trên LockScreen. |
| **2. Bảo Khố (Vault)** | **ĐÚNG** | `domain/model/VaultApp.kt`, `data/dao/VaultAppDao.kt`, `VaultScreen.kt`. | Đã hoàn thành 100% Canonical Mục 6: Khám phá app trên máy, thêm/gỡ app khỏi Bảo Khố, hiển thị dạng ô túi đồ, tuyệt đối không dùng icon ổ khóa, cascade xóa quan hệ khi gỡ app. | Hoàn thành trong Phase 19. |
...
| **7. App ↔ Task relationship** | **ĐÚNG** | `data/entity/TaskAppCrossRef.kt`, `TaskAppCrossRefDao.kt`, `SetTaskAppLinksUseCase.kt`. | Đã hỗ trợ quan hệ Many-to-Many giữa Task và App trong Bảo Khố (Mục 5, 6 Canonical). Cascade gỡ liên kết khi app bị xóa khỏi Bảo Khố. | Hoàn thành trong Phase 19. |
| **8. Multiple tasks per app** | **ĐÚNG** | `TaskAppCrossRefDao.kt:getTaskIdsForPackage`, `TaskAppEnforcementAdapter.kt`. | Hỗ trợ 1 app có thể gán nhiều task độc lập; adapter thu thập toàn bộ task liên kết để đánh giá trạng thái hoàn thành. | Hoàn thành trong Phase 19 & 20. |
| **9. Multiple apps per task** | **ĐÚNG** | `TaskAppCrossRefDao.kt:getPackageNamesForTask`, `TaskAppEnforcementAdapter.kt`. | Hỗ trợ 1 task có thể liên kết nhiều app; khi task hoàn thành, cập nhật đồng bộ cho tất cả app liên quan. | Hoàn thành trong Phase 19 & 20. |
| **10. Unlock rule** | **CHƯA XÁC ĐỊNH** | `TaskAppEnforcementAdapter.kt` phân định explicit `PENDING_OPEN_01`, giữ ranh giới không unlock. Phụ thuộc **OPEN-01**. | Thiếu công thức chính thức giải phong ấn (2/3 nhiệm vụ, rounding, special cases). Tuyệt đối không tự đoán hành vi trong code. | Giữ trạng thái an toàn đóng; chờ Ký chủ phê duyệt công thức OPEN-01. |
