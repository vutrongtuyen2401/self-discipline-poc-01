...
| **3. Nhiệm Vụ Đường (Mission Hall)** | **THIẾU** | Hoàn toàn không có file hay package nào liên quan đến task/mission trong dự án (0 code). | Thiếu toàn bộ phân hệ Nhiệm Vụ Đường theo Canonical Mục 5. | Triển khai phân hệ Nhiệm Vụ Đường ở các phase tiếp theo. |
| **4. Task creation** | **THIẾU** | Không có model Task hay UI tạo nhiệm vụ. | Thiếu nút thêm nhiệm vụ nhỏ góc dưới phải, thiếu form tối giản (chỉ gồm Tên + Nút xác nhận). | Triển khai theo Canonical Mục 5. |
| **5. Task completion** | **THIẾU** | `LockScreenActivity.kt` chỉ có nút quay lại an toàn (`onBackPressedDispatcher.onBackPressed()`). | Thiếu cơ chế xác nhận hoàn thành nhiệm vụ nằm trực tiếp trong luồng chặn tại app đích; thiếu logic tự ẩn task đã xong. | Tích hợp luồng xác nhận task vào `LockScreenActivity`. |
| **6. Task chain** | **THIẾU** | Không có cơ chế quản lý chuỗi task. | Thiếu hiển thị nhiệm vụ tuần tự, tự chuyển sang nhiệm vụ tiếp theo khi xong, tự bỏ qua nhiệm vụ bị xóa giữa chừng. | Xây dựng Task Chain Manager. |
...
| **11. Daily Cycle 04:00 reset** | **SAI** | `usage/UsageTracker.kt:155` (`getTodayDateString()` dùng `LocalDate.now()` reset lúc `00:00:00` nửa đêm). | **MÂU THUẪN NGHIỆP VỤ:** Canonical Design Mục 8 quy định chu kỳ ngày reset tại **04:00 sáng** (Giờ Dần). Task bắt đầu trước 04:00 vẫn thuộc chu kỳ cũ. | Sửa logic tính ngày nghiệp vụ sang mốc 04:00 sáng (`BusinessDayProvider`). |
